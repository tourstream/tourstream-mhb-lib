...
class Home extends React.Component {
  render () {
    ...
    if (t('hotels')) {
      const hotels = (typeof t('hotels') === 'string') ? JSON.parse(t('hotels')) : t('hotels')
          ...
          <Card header={`<h1>${t('home.teaser.locations.title')}</h1>`}>
            <LazyLoad height={600} offset={300}>
              <Map
                ...
                center={(typeof t('google.maps.center') === 'string') ? JSON.parse(t('google.maps.center')) : t('google.maps.center')}
                zoom={parseInt(t('google.maps.zoom'))}
                styles={(typeof t('google.maps.style') === 'string') ? JSON.parse(t('google.maps.style')) : t('google.maps.style')}
              />
            </LazyLoad>
          </Card>)
      }
    ...
    if (t('home.teaser.highlights.images')) {
      try {
        highlightsImages = (typeof t('home.teaser.highlights.images') === 'string') ? JSON.parse(t('home.teaser.highlights.images')) : t('home.teaser.highlights.images')
      } catch (e) {
        console.error('home.teaser.highlights.images', e.message)
      }
    }
    ...
    if (t('home.teaser.slider.images')) {
      try {
        sliderImages = (typeof t('home.teaser.slider.images') === 'string') ? JSON.parse(t('home.teaser.slider.images')) : t('home.teaser.slider.images')
      } catch (e) {
        console.error('home.teaser.slider.images', e.message)
      }
    }
