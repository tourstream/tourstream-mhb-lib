...
const StyledDropDown = styled.div`
  ...
  @media (min-width: 576px) {
    .dropdown-menu {
      ...
      color: ${props => props.theme.headerDropDownFontColor || 'white'} !important;
      ...
      <StyledDropDown className="dropdown">
        ...
        <div className={dropDownMenuClass} aria-labelledby="dropdownMenuButton">
          { Object.keys(settings.entries).map(key =>
            <Link key={key} className={`dropdown-item ${(key === settings.selected) ? 'active' : ''}`} to={settings.entries[key]} onClick={this.toggleClass}>{key}<i /></Link>) }
        </div>
      </StyledDropDown>
    )
  }
