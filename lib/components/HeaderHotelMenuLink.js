...
class HeaderHotelMenuLink extends Component {
  ...
  render () {
    ...
    if (Object.keys(hotels).length > 0) {
      ...
      let hotelLinks = []
      Object.keys(hotels).forEach(key => hotelLinks.push({ url: key, label: hotels[key] }))
      hotelLinks = hotelLinks.sort((a, b) => (a.label > b.label) ? 1 : -1)
        ...
        <li>
          <StyledDropDown className="dropdown">
            ...
            <div className={dropDownMenuClass} aria-labelledby="dropdownMenuButton">
              {hotelLinks.map(entry => this.renderLink(entry.url, entry.label))}
            </div>
          </StyledDropDown>
        </li>
      )
