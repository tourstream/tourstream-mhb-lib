...
class HeaderHotelMenuLink extends Component {
  ...
  render () {
    const { hotels, linkText } = this.props
    if (Object.keys(hotels).length > 0) {
      ...
      let hotelLinks = []
      Object.keys(hotels).forEach(key => hotelLinks.push({ label: key, url: hotels[key] }))
      hotelLinks = hotelLinks.sort((a, b) => (a.label.toLowerCase() > b.label.toLowerCase()) ? 1 : -1)
        ...
        <li>
          <StyledDropDown className="dropdown">
            ...
            <div className={dropDownMenuClass} aria-labelledby="dropdownMenuButton">
              {hotelLinks.map(entry => this.renderLink(entry.label, entry.url))}
            </div>
          </StyledDropDown>
        </li>
      )
