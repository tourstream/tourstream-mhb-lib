...
class ConsentPackage extends React.Component {
  ...
  componentWillReceiveProps (nextProps) {
    this.state.modalText = nextProps.modalText.replace('%brand%', nextProps.brandText)
  }
  ...
  render () {
    const {
      ...
      disableStyles, mainText, infoText, modalLinkText, modalHeader,
    } = this.props
    ...
    return (
      <div>
        <Modal
          ...
          little>
          <h2>{modalHeader}</h2>
          <span dangerouslySetInnerHTML={{ __html: this.state.modalText }} />
          <Link
            smooth
            style={astyle}
...
ConsentPackage.defaultProps = {
  ...
  infoText: 'more info',
  brandText: 'Meeting Point Hotels',
  buttonText: 'X',
